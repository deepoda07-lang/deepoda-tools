...
export default function VideoCompressPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Compress Video</h1>
      <p className="text-gray-500 dark:text-gray-400 mb-2">Reduce the size of MP4, MOV, and WEBM videos.</p>
      ...
      {file && (
        <div className="mt-6">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Quality</p>
          <div className="grid grid-cols-3 gap-2">
            {PRESETS.map((p, i) => (
              ...
              >
                <div>{p.label}</div>
                <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{p.desc}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {outputSize !== null && file && (
        <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-600 dark:text-gray-400">
          Original: <span className="font-medium text-gray-900 dark:text-white">{formatBytes(file.size)}</span>
          {" → "}
          Compressed: <span className="font-medium text-green-600 dark:text-green-400">{formatBytes(outputSize)}</span>
