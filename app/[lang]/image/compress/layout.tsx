...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const prefix = lang === "en" ? "" : `/${lang}`;
    const faq = dict.t.imgCompress.faq ?? [];
  const faqJsonLd = {
  ...
  const relatedItems = (RELATED["imgCompress"] ?? []).map((k) => ({
    ...
    <div className="max-w-4xl mx-auto px-4 pt-6">
      ...
      <ToolSEOSection title={dict.t.imgCompress.title} desc={dict.t.imgCompress.meta.description} keywords={dict.t.imgCompress.meta.keywords} />
      <ToolFAQ items={faq} />
