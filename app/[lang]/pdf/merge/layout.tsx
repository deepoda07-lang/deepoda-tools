...
export default async function Layout({
  ...
  const toolTitle = dict.t.pdfMerge.title;
  const prefix = lang === "en" ? "" : `/${lang}`;

    const faq = dict.t.pdfMerge.faq ?? [];
  const faqJsonLd = {
  ...
  const relatedItems = (RELATED["pdfMerge"] ?? []).map((k) => ({
    href: TOOL_HREF[k],
    ...
    <div className="max-w-4xl mx-auto px-4 pt-6">
      ...
      <ToolSEOSection title={dict.t.pdfMerge.title} desc={dict.t.pdfMerge.meta.description} keywords={dict.t.pdfMerge.meta.keywords} />
      <ToolFAQ items={faq} />
